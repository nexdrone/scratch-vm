...
class Scratch3UdlTello {
    ...
    /**
     * window.csEvent が定義されているかどうか確認する
     */
    _checkCsEvent () {
        if (!window.csEvent) {
            alert('window.csEvent does not exist.');
            return false;
        }
        return true;
    }
    ...
    getBrowser () {
        ...
        window.csEvent.raiseEvent('click', JSON.stringify({
            data1: 'memo',
        }));
    }
    ...
    takeoff () {
        ...
        {
            boundAsync.takeoff().then(function (result)
            {
                const obj = JSON.parse(result);
                if (!result.status) {
                    alert(telloNotConnectMessage);
                }
            });
        });
    }
     ...
     */
    land () {
        //if (!this._checkCsEvent()){ return; }
        //window.csEvent.raiseEvent('land', JSON.stringify({
        //}));
        CefSharp.BindObjectAsync("boundAsync").then(function(result)
        {
            boundAsync.land().then(function (result)
            {
                const obj = JSON.parse(result);
                if (!result.status) {
                    alert(telloNotConnectMessage);
